        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # pozyskaj variant produktu po id koszyka
...
def my_orders(request):
    orders = Order.objects.filter(user=request.user, is_ordered=True).order_by('-created_at') #descending order
    context = {
        'orders': orders,
    }
...
def change_password(request):
    if request.method == "POST":
        ...
        if new_password == confirm_password:
            success = user.check_password(current_password) #check hashed password
            if success:
                ...
                messages.error(request, "Poprawnie wpisz aktualne hasło")
                return redirect('change_password')
        else:
...
def order_detail(request, order_id):
    try:
        order_detail = OrderProduct.objects.filter(order__order_number=order_id) #__access foregin key model OrderProduct --> Order
